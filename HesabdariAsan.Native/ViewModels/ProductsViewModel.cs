...
public sealed class ProductsViewModel : PagedViewModelBase
{
    ...
    public IReadOnlyList<string> FilterOptions { get; }=new[]{"همه کالاها","موجودی کم","تبدیل واحد فعال"};
    ...
    {
        AddCommand=new RelayCommand(_=>Add()); EditCommand=new RelayCommand(x=>Edit(x as Product),x=>x is Product); DeleteCommand=new RelayCommand(x=>Delete(x as Product),x=>x is Product); PurchaseCommand=new RelayCommand(x=>Purchase(x as Product),x=>x is Product); ReloadCategories();Reload();
    }
    public override void Reload()
    {
        var cat=SelectedCategory=="همه دسته‌ها"?"ALL":SelectedCategory; var filter=SelectedFilter=="موجودی کم"?"LOW":SelectedFilter=="تبدیل واحد فعال"?"PACKAGED":"ALL";
