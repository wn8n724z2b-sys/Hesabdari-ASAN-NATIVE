...
{
    private readonly DebounceDispatcher _searchDebounce = new(TimeSpan.FromMilliseconds(220));
    ...
    public int PageSize { get=>_pageSize; set{if(Set(ref _pageSize,value)){_searchDebounce.Cancel();Page=1;Reload();}} }
    ...
    public string Search { get=>_search; set{if(Set(ref _search,value)){Page=1;_searchDebounce.Schedule(Reload);}} }
    ...
    protected PagedViewModelBase(){PreviousCommand=new RelayCommand(_=>{if(CanPrevious){_searchDebounce.Cancel();Page--;Reload();}});NextCommand=new RelayCommand(_=>{if(CanNext){_searchDebounce.Cancel();Page++;Reload();}});}
