...
{
    public PagedResult<InvoiceSummary> Search(string? query,int page,int pageSize)
    {
        page=Math.Max(1,page);pageSize=pageSize is 10 or 20 or 50? pageSize:10;query=(query??"").Trim();
        var where=query.Length==0?"":"WHERE customer_name LIKE $like OR CAST(invoice_no AS TEXT) LIKE $like";
        using var db=Database.Open();int total;using(var c=db.CreateCommand()){c.CommandText=$"SELECT COUNT(*) FROM invoices {where}";if(query.Length>0)c.Parameters.AddWithValue("$like",$"%{query}%");total=Convert.ToInt32(c.ExecuteScalar()??0);}
