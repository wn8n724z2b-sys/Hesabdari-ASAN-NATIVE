...
public sealed class ReceiptPrinterService
{
    ...
    public bool PrintTest(string? printerName = null)
    {
        var dialog = new PrintDialog();
        var queue = FindQueue(string.IsNullOrWhiteSpace(printerName) ? _settings.Get("printer_name", "") : printerName);
        if (queue is null)
        {
            if (dialog.ShowDialog() != true) return false;
        }
        else dialog.PrintQueue = queue;
        var doc = new FlowDocument { PageWidth = 302, PageHeight = 500, PagePadding = new Thickness(10), ColumnWidth = 282, FlowDirection = FlowDirection.RightToLeft, FontFamily = new FontFamily("Nirmala UI"), FontSize = 12, Foreground = Brushes.Black };
        doc.Blocks.Add(new Paragraph(new Run("حسابداری آسان")) { TextAlignment = TextAlignment.Center, FontSize = 18, FontWeight = FontWeights.Bold });
        doc.Blocks.Add(new Paragraph(new Run("چاپ آزمایشی 80mm")) { TextAlignment = TextAlignment.Center, FontWeight = FontWeights.SemiBold });
        doc.Blocks.Add(new Paragraph(new Run(DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss"))) { TextAlignment = TextAlignment.Center, FlowDirection = FlowDirection.LeftToRight });
        doc.Blocks.Add(new Paragraph(new Run("پرینتر با موفقیت از مسیر Native Windows دریافت شد.")) { TextAlignment = TextAlignment.Center });
        dialog.PrintDocument(((IDocumentPaginatorSource)doc).DocumentPaginator, "حسابداری آسان - چاپ آزمایشی");
        return true;
    }
    ...
    private static FlowDocument BuildReceipt(InvoiceDetail invoice, AppSettings settings)
    {
        ...
        foreach (var item in invoice.Items)
        {
            ...
            doc.Blocks.Add(P($"{item.QuantityText} {item.Unit} × {item.UnitPrice:N0} = {item.LineTotal:N0} ؋", 10));
        }
