...
public sealed class V34MigrationService
{
    ...
    public MigrationResult Import(string sourcePath, bool replaceCurrent)
    {
        ...
        {
            cmd.CommandText = @"SELECT p.id,p.name,p.category_id,p.base_unit,p.average_unit_cost,p.sell_price,p.stock,p.min_stock,p.is_active,p.created_at,
COALESCE((SELECT barcode FROM product_barcodes b WHERE b.product_id=p.id ORDER BY b.is_primary DESC,b.id LIMIT 1),'')
FROM products p ORDER BY p.created_at,p.id";
            using var r = cmd.ExecuteReader();
            while (r.Read())
            {
                var legacyId = r.GetString(0);
                var barcode = r.GetString(10);
                using var ins = target.CreateCommand(); ins.Transaction = tx;
                ins.CommandText = @"INSERT INTO products(name,barcode,category,unit,purchase_price,sale_price,stock,min_stock,is_active,created_at)
VALUES($n,$b,$c,$u,$pp,$sp,$st,$ms,$active,$at); SELECT last_insert_rowid();";
                ins.Parameters.AddWithValue("$n", r.GetString(1));
                var categoryId = r.IsDBNull(2) ? "" : r.GetString(2);
                ...
                ins.Parameters.AddWithValue("$u", r.IsDBNull(3) ? "عدد" : r.GetString(3));
                ins.Parameters.AddWithValue("$pp", Money(r.GetDouble(4)));
                ins.Parameters.AddWithValue("$sp", Money(r.GetDouble(5)));
                ins.Parameters.AddWithValue("$st", r.GetDouble(6));
                ins.Parameters.AddWithValue("$ms", r.GetDouble(7));
                ins.Parameters.AddWithValue("$active", r.GetInt64(8));
                ins.Parameters.AddWithValue("$at", r.GetString(9));
                var nativeId = Convert.ToInt64(ins.ExecuteScalar() ?? 0L);
                productMap[legacyId] = nativeId;
                productBarcodes[legacyId] = barcode;
...
VALUES($no,$cid,$cn,$pt,$sub,$dis,$tot,$paid,$at); SELECT last_insert_rowid();";
        ...
        using (var cmd = source.CreateCommand())
        {
            cmd.CommandText = "SELECT sale_id,product_id,product_name,qty,sell_price,unit_cost FROM sale_items ORDER BY id";
            using var r = cmd.ExecuteReader();
            while (r.Read())
            {
                ...
                var qty = r.GetDouble(3); var unitPrice = Money(r.GetDouble(4));
                using var ins = target.CreateCommand(); ins.Transaction = tx;
                ins.CommandText = @"INSERT INTO invoice_items(invoice_id,product_id,product_name,barcode,qty,unit_price,purchase_price,line_total)
VALUES($iid,$pid,$n,$b,$q,$up,$pp,$lt)";
                ...
                ins.Parameters.AddWithValue("$pp", Money(r.GetDouble(5)));
                ins.Parameters.AddWithValue("$lt", Money(qty * r.GetDouble(4)));
...
DELETE FROM supplier_payments; DELETE FROM purchases; DELETE FROM inventory_ledger; DELETE FROM expenses; DELETE FROM products;
DELETE FROM parties; DELETE FROM financial_periods; DELETE FROM audit_log; DELETE FROM legacy_id_map;";
    ...
    {
        using var cmd=source.CreateCommand();cmd.CommandText="SELECT purchased_at,product_id,product_name,quantity,units,unit_cost,total,payment_method,supplier_id FROM purchases ORDER BY purchased_at";using var r=cmd.ExecuteReader();
        while(r.Read()){var legacyProduct=r.GetString(1);long? productId=products.TryGetValue(legacyProduct,out var p)?p:null;var legacySupplier=r.IsDBNull(8)?"":r.GetString(8);long? supplierId=parties.TryGetValue(legacySupplier,out var s)?s:null;using var ins=target.CreateCommand();ins.Transaction=tx;ins.CommandText=@"INSERT INTO purchases(product_id,product_name,quantity,units,unit_cost,total,payment_type,supplier_id,created_at) VALUES($p,$n,$q,$u,$uc,$t,$pt,$s,$at)";ins.Parameters.AddWithValue("$p",productId.HasValue?productId.Value:DBNull.Value);ins.Parameters.AddWithValue("$n",r.GetString(2));ins.Parameters.AddWithValue("$q",r.GetDouble(3));ins.Parameters.AddWithValue("$u",r.GetDouble(4));ins.Parameters.AddWithValue("$uc",Money(r.GetDouble(5)));ins.Parameters.AddWithValue("$t",Money(r.GetDouble(6)));ins.Parameters.AddWithValue("$pt",PaymentType(r.GetString(7)));ins.Parameters.AddWithValue("$s",supplierId.HasValue?supplierId.Value:DBNull.Value);ins.Parameters.AddWithValue("$at",r.GetString(0));ins.ExecuteNonQuery();result.Purchases++;}
    }
