...
public sealed class SalesService
{
    public long GetNextInvoiceNo()
    {
        ...
        cmd.CommandText = "SELECT COALESCE(MAX(invoice_no),0)+1 FROM invoices";
...
VALUES($no,$cid,$cn,$pt,$sub,$dis,$tot,$paid,'ACTIVE',$at);SELECT last_insert_rowid();";
        ...
        foreach(var item in items)
        {
            using(var line=db.CreateCommand()){line.Transaction=tx;line.CommandText=@"INSERT INTO invoice_items(invoice_id,product_id,product_name,barcode,qty,unit_price,purchase_price,line_total,unit)
VALUES($iid,$pid,$n,$b,$q,$up,$pp,$lt,$unit)";line.Parameters.AddWithValue("$iid",invoiceId);line.Parameters.AddWithValue("$pid",item.ProductId);line.Parameters.AddWithValue("$n",item.Name);line.Parameters.AddWithValue("$b",item.Barcode);line.Parameters.AddWithValue("$q",item.Quantity);line.Parameters.AddWithValue("$up",item.UnitPrice);line.Parameters.AddWithValue("$pp",item.PurchasePrice);line.Parameters.AddWithValue("$lt",item.LineTotal);line.Parameters.AddWithValue("$unit",item.Unit);line.ExecuteNonQuery();}
