...
{
    public PagedResult<Expense> Search(string? query,int page,int pageSize)
    {
        page=Math.Max(1,page); pageSize=pageSize is 10 or 20 or 50 ? pageSize : 10; query=(query??"").Trim();
        var where=query.Length==0?"":"WHERE title LIKE $like OR category LIKE $like OR note LIKE $like"; using var db=Database.Open(); int total;
        using(var count=db.CreateCommand()){count.CommandText=$"SELECT COUNT(*) FROM expenses {where}";if(query.Length>0)count.Parameters.AddWithValue("$like",$"%{query}%");total=Convert.ToInt32(count.ExecuteScalar()??0);}
        var items=new List<Expense>(); using var cmd=db.CreateCommand(); cmd.CommandText=$@"SELECT id,title,category,amount,COALESCE(note,''),created_at FROM expenses {where} ORDER BY created_at DESC,id DESC LIMIT $l OFFSET $o"; if(query.Length>0)cmd.Parameters.AddWithValue("$like",$"%{query}%");cmd.Parameters.AddWithValue("$l",pageSize);cmd.Parameters.AddWithValue("$o",(page-1)*pageSize);
