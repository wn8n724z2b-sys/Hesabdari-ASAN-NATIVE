...
public sealed class InvoiceAdjustmentService
{
    public InvoiceEditData LoadForEdit(long invoiceNo)
    {
        ...
        foreach (var x in detail.Items)
        {
            ...
            {
                ProductId = x.ProductId, ProductName = x.ProductName, Barcode = x.Barcode, Unit = x.Unit,
    ...
    public void SaveEdit(InvoiceEditData edited)
    {
        ...
        foreach (var item in edited.Items)
        {
            ...
            {
                line.Transaction = tx;
                line.CommandText = @"INSERT INTO invoice_items(invoice_id,product_id,product_name,barcode,qty,unit_price,purchase_price,line_total,unit)
VALUES($iid,$pid,$name,$barcode,$qty,$price,$purchase,$total,$unit)";
                ...
                line.Parameters.AddWithValue("$purchase", item.PurchasePrice); line.Parameters.AddWithValue("$total", item.LineTotal); line.Parameters.AddWithValue("$unit", item.Unit); line.ExecuteNonQuery();
            }
...
FROM invoices WHERE invoice_no=$no LIMIT 1";
        ...
        using (var cmd = db.CreateCommand())
        {
            cmd.Transaction = tx; cmd.CommandText = @"SELECT product_id,product_name,COALESCE(barcode,''),qty,unit_price,purchase_price,line_total,COALESCE(unit,'دانه') FROM invoice_items WHERE invoice_id=$id ORDER BY id"; cmd.Parameters.AddWithValue("$id", x.Id);
            using var r = cmd.ExecuteReader();
            while (r.Read()) x.Items.Add(new SnapshotItem { ProductId = r.IsDBNull(0) ? null : r.GetInt64(0), ProductName = r.GetString(1), Barcode = r.GetString(2), Quantity = r.GetDouble(3), UnitPrice = r.GetInt64(4), PurchasePrice = r.GetInt64(5), LineTotal = r.GetInt64(6), Unit = r.GetString(7) });
        }
        return x;
    }
