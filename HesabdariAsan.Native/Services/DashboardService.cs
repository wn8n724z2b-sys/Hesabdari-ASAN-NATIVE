...
public sealed class DashboardService
{
    public DashboardMetrics Load()
    {
        ...
        long Scalar(string sql, bool day = false)
        {
            using var c = db.CreateCommand();
            c.CommandText = sql;
            if (day) c.Parameters.AddWithValue("$d", start);
...
FROM invoice_items ii JOIN invoices i ON i.id=ii.invoice_id WHERE COALESCE(i.status,'ACTIVE')='ACTIVE' AND i.created_at >= $d", true);
        var invoiceCount = Scalar("SELECT COUNT(*) FROM invoices WHERE COALESCE(status,'ACTIVE')='ACTIVE' AND created_at >= $d", true);
        var lowStock = Scalar("SELECT COUNT(*) FROM products WHERE is_active=1 AND stock <= min_stock");
        var customers = Scalar("SELECT COUNT(*) FROM parties WHERE type='CUSTOMER'");
        ...
        return new DashboardMetrics
        {
            ...
            TodayNetProfit = gross - expenses,
            TodaySalesCount = invoiceCount,
    ...
    public IReadOnlyList<InvoiceSummary> RecentInvoices(int limit = 10)
    {
        ...
        cmd.CommandText = @"SELECT id,invoice_no,customer_name,payment_type,total,COALESCE(status,'ACTIVE'),created_at FROM invoices ORDER BY created_at DESC,id DESC LIMIT $l";
        cmd.Parameters.AddWithValue("$l", Math.Clamp(limit, 1, 50));
        ...
        while (r.Read())
            list.Add(new InvoiceSummary
            {
                Id = r.GetInt64(0),
                RowNumber = ++i,
                InvoiceNo = r.GetInt64(1),
                CustomerName = r.GetString(2),
                PaymentType = r.GetString(3),
                Total = r.GetInt64(4),
