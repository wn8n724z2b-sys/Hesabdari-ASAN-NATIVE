...
public partial class MainWindow : Window
{
    ...
    public MainWindow()
    {
        ...
        Loaded+=(_,_)=>{_workSession.Start();Tick();_clockTimer.Tick+=(_,_)=>Tick();_clockTimer.Start();_pulseTimer.Tick+=(_,_)=>_workSession.Pulse();_pulseTimer.Start();_autoBackup.Check();_backupTimer.Tick+=(_,_)=>_autoBackup.Check();_backupTimer.Start();};
        Closing+=(_,_)=>{_scanner.Dispose();_backupTimer.Stop();_pulseTimer.Stop();_clockTimer.Stop();_workSession.Stop();};
    }
    ...
    private void AttachScanner()
    {
        ...
        {
            // A keyboard wedge scanner still works through the focused search field if Raw Input is unavailable.
        }
    }

    private void Tick(){ClockText.Text=DateTime.Now.ToString("HH:mm:ss");WorkText.Text=_workSession.GetTodayDuration().ToString(@"hh\:mm\:ss");}
}
