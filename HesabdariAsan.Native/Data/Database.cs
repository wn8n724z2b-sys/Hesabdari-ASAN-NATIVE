...
CREATE TABLE IF NOT EXISTS purchases (
    ...
    pack_cost INTEGER NOT NULL DEFAULT 0,
...
";
        ...
        EnsureColumn(db, "purchases", "mode", "TEXT NOT NULL DEFAULT 'BASE'");
        EnsureColumn(db, "purchases", "purchase_unit", "TEXT NOT NULL DEFAULT 'دانه'");
        EnsureColumn(db, "purchases", "units_per_purchase", "REAL NOT NULL DEFAULT 1");
        EnsureColumn(db, "purchases", "pack_cost", "INTEGER NOT NULL DEFAULT 0");
        EnsureColumn(db, "purchases", "average_cost", "INTEGER NOT NULL DEFAULT 0");
        EnsureColumn(db, "invoice_items", "unit", "TEXT NOT NULL DEFAULT 'دانه'");
        EnsureColumn(db, "products", "producer", "TEXT NOT NULL DEFAULT ''");
        EnsureColumn(db, "products", "image_path", "TEXT NULL");
        EnsureColumn(db, "products", "unit_conversion_enabled", "INTEGER NOT NULL DEFAULT 0");
        EnsureColumn(db, "products", "base_unit", "TEXT NOT NULL DEFAULT 'دانه'");
        EnsureColumn(db, "products", "purchase_unit", "TEXT NOT NULL DEFAULT 'دانه'");
        EnsureColumn(db, "products", "units_per_purchase", "REAL NOT NULL DEFAULT 1");
        EnsureColumn(db, "products", "package_buy_price", "INTEGER NOT NULL DEFAULT 0");
        using (var normalize = db.CreateCommand())
        {
            normalize.CommandText = @"UPDATE products SET base_unit=CASE WHEN trim(COALESCE(base_unit,''))='' THEN COALESCE(NULLIF(unit,''),'دانه') ELSE base_unit END,
unit=CASE WHEN trim(COALESCE(unit,''))='' THEN COALESCE(NULLIF(base_unit,''),'دانه') ELSE unit END,
purchase_unit=CASE WHEN trim(COALESCE(purchase_unit,''))='' THEN COALESCE(NULLIF(base_unit,''),'دانه') ELSE purchase_unit END,
units_per_purchase=CASE WHEN units_per_purchase<=0 THEN 1 ELSE units_per_purchase END,
package_buy_price=CASE WHEN package_buy_price<=0 AND unit_conversion_enabled=0 THEN purchase_price ELSE package_buy_price END;";
            normalize.ExecuteNonQuery();
        }
        using (var seedBarcodes = db.CreateCommand())
        {
            seedBarcodes.CommandText = @"INSERT OR IGNORE INTO product_barcodes(product_id,barcode,is_primary)
SELECT id,barcode,1 FROM products WHERE trim(COALESCE(barcode,''))<>'';";
            seedBarcodes.ExecuteNonQuery();
        }
