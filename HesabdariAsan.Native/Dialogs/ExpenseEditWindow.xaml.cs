...
public partial class ExpenseEditWindow : Window
{
    ...
    private void Save_Click(object sender,RoutedEventArgs e){if(string.IsNullOrWhiteSpace(NameBox.Text)){AppDialog.Show("عنوان هزینه را وارد کنید.");return;}if(!long.TryParse(AmountBox.Text,out var amount)||amount<0){AppDialog.Show("مبلغ درست نیست.");return;}Expense.Title=NameBox.Text.Trim();Expense.Category=string.IsNullOrWhiteSpace(CategoryBox.Text)?"هزینه":CategoryBox.Text.Trim();Expense.Amount=amount;Expense.Note=NoteBox.Text.Trim();DialogResult=true;}
